...
class Steam_User_Stats : public ISteamUserStats003,
  ...
  private:
    ...
    std::vector<Steam_Leaderboard_Score> load_leaderboard_scores(std::string name) {
        ...
        if (local_storage->get_data(Local_Storage::leaderboard_storage_folder, leaderboard_name, (char *)output.data(), size) != size)
            return out;
  ...
  public:
    Steam_User_Stats(Settings *settings, Local_Storage *local_storage, class SteamCallResults *callback_results, class SteamCallBacks *callbacks, Steam_Overlay *overlay) : settings(settings),
                                                                                                                                                                            local_storage(local_storage),
    ...
    bool GetStat(const char *pchName, int32 *pData) {
        ...
        if (stats_data != stats_config.end()) {
            if (stats_data->second.type != Stat_Type::STAT_TYPE_INT)
                return false;
        }
    ...
    SteamAPICall_t DownloadLeaderboardEntries(SteamLeaderboard_t hSteamLeaderboard, ELeaderboardDataRequest eLeaderboardDataRequest, int nRangeStart, int nRangeEnd) {
        ...
        std::lock_guard<std::recursive_mutex> lock(global_mutex);
        if (hSteamLeaderboard > leaderboards.size() || hSteamLeaderboard <= 0)
            return k_uAPICallInvalid; // might return callresult even if hSteamLeaderboard is invalid
...
bool GetDownloadedLeaderboardEntry(SteamLeaderboardEntries_t hSteamLeaderboardEntries, int index, LeaderboardEntry_t *pLeaderboardEntry, int32 *pDetails, int cDetailsMax) {
    ...
    std::lock_guard<std::recursive_mutex> lock(global_mutex);
    if (hSteamLeaderboardEntries > leaderboards.size() || hSteamLeaderboardEntries <= 0)
        return false;
    if (index > 0)
        return false;
...
SteamAPICall_t UploadLeaderboardScore(SteamLeaderboard_t hSteamLeaderboard, ELeaderboardUploadScoreMethod eLeaderboardUploadScoreMethod, int32 nScore, const int32 *pScoreDetails, int cScoreDetailsCount) {
    ...
    std::lock_guard<std::recursive_mutex> lock(global_mutex);
    if (hSteamLeaderboard > leaderboards.size() || hSteamLeaderboard <= 0)
        return k_uAPICallInvalid; // TODO: might return callresult even if hSteamLeaderboard is invalid
