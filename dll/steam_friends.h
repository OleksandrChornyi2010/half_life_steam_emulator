...
class Steam_Friends : public ISteamFriends003,
  ...
  public:
    ...
    Steam_Friends(Settings *settings, Networking *network, SteamCallResults *callback_results, SteamCallBacks *callbacks, RunEveryRunCB *run_every_runcb, Steam_Overlay *overlay) : settings(settings),
                                                                                                                                                                                    network(network),
                                                                                                                                                                                    callbacks(callbacks),
    ...
    EFriendRelationship GetFriendRelationship(CSteamID steamIDFriend) {
        ...
        if (steamIDFriend == settings->get_local_steam_id())
            return k_EFriendRelationshipNone; // Real steam behavior
        if (find_friend(steamIDFriend))
    ...
    bool GetFriendGamePlayed(CSteamID steamIDFriend, uint64 *pulGameID, uint32 *punGameIP, uint16 *pusGamePort, uint16 *pusQueryPort) {
        ...
        if (ret) {
            if (pulGameID)
                *pulGameID = info.m_gameID.ToUint64();
            if (punGameIP)
                *punGameIP = info.m_unGameIP;
            if (pusGamePort)
                *pusGamePort = info.m_usGamePort;
            if (pusQueryPort)
                *pusQueryPort = info.m_usQueryPort;
        }
    ...
    const char *GetFriendPersonaNameHistory(CSteamID steamIDFriend, int iPersonaName) {
        ...
        const char *ret = "";
        if (iPersonaName == 0)
            ret = GetFriendPersonaName(steamIDFriend);
        else if (iPersonaName == 1)
            ret = "Some Old Name";
    ...
    bool IsUserInSource(CSteamID steamIDUser, CSteamID steamIDSource) {
        ...
        if (steamIDUser == settings->get_local_steam_id()) {
            ...
            if (!f)
                return false;
            if (f->lobby_id() == steamIDSource.ConvertToUint64())
                return true;
        }
        // TODO
        return false;
    ...
    void RequestFriendRichPresence(CSteamID steamIDFriend) {
        ...
        Friend *f = find_friend(steamIDFriend);
        if (f)
            rich_presence_updated(steamIDFriend, settings->get_local_game_id().AppID());
    }
