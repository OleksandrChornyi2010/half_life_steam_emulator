...
struct Controller_Action {
    ...
    std::set<int> button_id(ControllerDigitalActionHandle_t handle) {
        ...
        if (a == active_map.active_digital.end())
            return {};
        return a->second;
    }
    ...
    std::pair<std::set<int>, enum EInputSourceMode> analog_id(ControllerAnalogActionHandle_t handle) {
        ...
        if (a == active_map.active_analog.end())
            return std::pair<std::set<int>, enum EInputSourceMode>({}, k_EInputSourceMode_None);
        return a->second;
    }
};
...
class Steam_Controller : public ISteamController001,
  ...
  public:
        ...
        for (auto button : buttons) {
            ...
            if (button < BUTTON_COUNT) {
                ...
                case BUTTON_STICK_LEFT_RIGHT: {
                    ...
                    x *= len;
                    y *= len;
                    if (button == BUTTON_STICK_LEFT_UP)
                        pressed = y > DEADZONE_BUTTON_STICK;
                    if (button == BUTTON_STICK_LEFT_DOWN)
                        pressed = y < -DEADZONE_BUTTON_STICK;
                    if (button == BUTTON_STICK_LEFT_RIGHT)
                        pressed = x > DEADZONE_BUTTON_STICK;
                    if (button == BUTTON_STICK_LEFT_LEFT)
                        pressed = x < -DEADZONE_BUTTON_STICK;
                    break;
                }
                ...
                case BUTTON_STICK_RIGHT_RIGHT: {
                    ...
                    x *= len;
                    y *= len;
                    if (button == BUTTON_STICK_RIGHT_UP)
                        pressed = y > DEADZONE_BUTTON_STICK;
                    if (button == BUTTON_STICK_RIGHT_DOWN)
                        pressed = y < -DEADZONE_BUTTON_STICK;
                    if (button == BUTTON_STICK_RIGHT_RIGHT)
                        pressed = x > DEADZONE_BUTTON_STICK;
                    if (button == BUTTON_STICK_RIGHT_LEFT)
                        pressed = x < -DEADZONE_BUTTON_STICK;
                    break;
                }
                default:
    ...
    int GetAnalogActionOrigins(InputHandle_t inputHandle, InputActionSetHandle_t actionSetHandle, InputAnalogActionHandle_t analogActionHandle, EInputActionOrigin *originsOut) {
        ...
        if (a == map->second.active_analog.end())
            return 0;
    ...
    void TriggerVibration(ControllerHandle_t controllerHandle, unsigned short usLeftSpeed, unsigned short usRightSpeed) {
        ...
        if (gamepad_device > GAMEPAD_COUNT)
            return;
        rumble_thread_data->rumble_mutex.lock();
    ...
    const char *GetGlyphForActionOrigin(EControllerActionOrigin eOrigin) {
        ...
        if (glyph == steamcontroller_glyphs.end())
            return "";
    ...
    const char *GetGlyphForActionOrigin(EInputActionOrigin eOrigin) {
        ...
        if (glyph == steaminput_glyphs.end())
            return "";
        return glyph->second.c_str();
    }
