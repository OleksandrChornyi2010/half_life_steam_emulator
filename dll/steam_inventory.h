...
class Steam_Inventory : public ISteamInventory001,
  ...
  public:
    ...
    Steam_Inventory(class Settings *settings, class SteamCallResults *callback_results, class SteamCallBacks *callbacks, class RunEveryRunCB *run_every_runcb, class Local_Storage *local_storage) : settings(settings),
    ...
    uint32 GetResultTimestamp(SteamInventoryResult_t resultHandle) {
        ...
        std::lock_guard<std::recursive_mutex> lock(global_mutex);
        struct Steam_Inventory_Requests *request = get_inventory_result(resultHandle);
        if (!request || !request->result_done())
            return 0;
