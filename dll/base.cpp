...
void set_whitelist_ips(uint32_t *from, uint32_t *to, unsigned num_ips) {
    ...
    for (unsigned i = 0; i < num_ips; ++i) {
        ...
        if (ip_a.ip_to < ip_a.ip_from)
            continue;
        if ((ip_a.ip_to - ip_a.ip_from) > (1 << 25))
            continue;
        PRINT_DEBUG("added\n");
...
static bool is_lan_ipv4(unsigned char *ip) {
    ...
    if (ip[0] == 192 && (ip[1] == 18 || ip[1] == 19))
        return true; // Used for benchmark testing of inter-network communications between two separate subnets.
    if (ip[0] >= 224)
        return true; // ip multicast (224 - 239) future use (240.0.0.0 - 255.255.255.254) broadcast (255.255.255.255)
    return false;
}
...
static bool is_lan_ip(const sockaddr *addr, int namelen) {
    ...
    if (addr->sa_family == AF_INET) {
        ...
        if (((ip[0] == 0xFF) && (ip[1] < 3) && (ip[15] == 1)) ||
            ...
            return true;
        if (memcmp(zeroes, ip, sizeof(ip)) == 0)
            return true;
        if (memcmp(zeroes, ip, sizeof(ip) - 1) == 0 && ip[15] == 1)
            return true;
        if (ip[0] == 0xff)
            return true; // multicast
        if (ip[0] == 0xfc)
...
static void load_dlls() {
    ...
    for (auto &p : paths) {
        ...
        if (length < 4)
            continue;
        if (std::toupper(full_path[length - 1]) != 'L')
            continue;
        if (std::toupper(full_path[length - 2]) != 'L')
            continue;
        if (std::toupper(full_path[length - 3]) != 'D')
            continue;
