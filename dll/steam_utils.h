...
class Steam_Utils : public ISteamUtils002,
  ...
  public:
    Steam_Utils(Settings *settings, class SteamCallResults *callback_results, Steam_Overlay *overlay) : settings(settings),
    ...
    bool GetImageRGBA(int iImage, uint8 *pubDest, int nDestBufferSize) {
        ...
        if (!iImage || !pubDest || !nDestBufferSize)
            return false;
        std::lock_guard<std::recursive_mutex> lock(global_mutex);
    ...
    bool IsAPICallCompleted(SteamAPICall_t hSteamAPICall, bool *pbFailed) {
        ...
        std::lock_guard<std::recursive_mutex> lock(global_mutex);
        if (!callback_results->exists(hSteamAPICall))
            return false;
        if (pbFailed)
            *pbFailed = false;
    ...
    int FilterText(ETextFilteringContext eContext, CSteamID sourceSteamID, const char *pchInputMessage, char *pchOutFilteredText, uint32 nByteSizeOutFilteredText) {
        ...
        if (!len)
            return 0;
        len += 1;
        if (len > nByteSizeOutFilteredText)
            len = nByteSizeOutFilteredText;
        len -= 1;
